...
from src.expred import ExpredInput, Expred
...
def get_loss_with_weight_decay(
    ...
    if preds['cls_pred'][0][0] > preds['cls_pred'][0][1]:
...
def compute_gradients(
    ...
    mtl_grad = torch.autograd.grad(
        outputs=loss,
        inputs=[
            param for name, param
            in model.mtl_module.named_parameters()
...
def compute_hessian_vector_products(
    ...
    # print("hessian_vector_products.loss: ", loss)
    ...
    model.zero_grad()
    # print("------------------------------")
    # print("grad_tuple: ", type(grad_tuple[3]))
    # print("inputs: ", type([
    #         param for name, param
    #         in model.named_parameters()
    #         if name not in params_filter][3]))
    # print("vectors: ", type(vectors[3]))
    # print("------------------------------")
...
def compute_s_test(
        ...
        device: torch.device,
        model: torch.nn.Module,
        test_inputs: ExpredInput, #Dict[str, torch.Tensor],
        ...
        for data_loader in train_data_loaders:
            for i, inputs in enumerate(data_loader):
                ...
                with torch.no_grad():
                    new_estimate = [
                        a + (1 - damp) * b - c / scale
                        for a, b, c in zip(v[0:2], last_estimate[0:2], this_estimate[0:2])
                    ]
