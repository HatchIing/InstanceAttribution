...
from typing import Dict, List, Set, Tuple, Union, Set
...
class Annotation:
    ...
    evidences: Union[Set[Tuple[Evidence]], Set[Tuple[Evidence]]]
    classification: str
    query_type: str = None
    docids: Set[str] = None
...
def _annotation_to_dict(dc):
    ...
    if is_dataclass(dc):
        ...
        return dc
    elif isinstance(dc, (set, set, list, tuple)):
        ret = []
        for x in dc:
...
def annotations_from_jsonl(fp: str) -> List[Annotation]:
    ...
    with open(fp, 'r') as inf:
        for line in inf:
            ...
            content['evidences'] = set(ev_groups)
...
def load_datasets(data_dir: str) -> Tuple[List[Annotation], List[Annotation], List[Annotation]]:
    ...
    """
    train_data = annotations_from_jsonl(os.path.join(data_dir, 'train_trimmed.jsonl'))
